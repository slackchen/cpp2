...
// 混动转义连接(M11):native 卸载的 cxx_legacy → 自包含 DLL TU。
// prelude 与转译全局域同面(legacy 原文可引用同一批标准头/rt 头),
// 转发器符号 cpp2leg_<name> 与 native 调用点、PE 导入表三方一致。
std::string emit_legacy_dll(hybrid::Plan const& plan)
{
    std::string out = "// Generated by cpp2c (native hybrid legacy DLL). DO NOT EDIT.\n";
    out += "// module: " + plan.module_name + " -> " + plan.dll_name + "\n";
    out += prelude_includes() + "\n";

    for (auto& [line, code] : plan.blocks) {
        out += "#line " + std::to_string(line) + " \"" + escape_path(plan.src_path) + "\"\n";
        out += code;
        out += "\n";
    }

    for (auto& ex : plan.exports) {
        std::string sym = hybrid::import_symbol(ex.name);
        out += "\nextern \"C\" __declspec(dllexport)\n";
        out += "long long " + sym + "(";
        for (size_t i = 0; i < ex.param_c.size(); ++i) {
            if (i) out += ", ";
            out += "long long p" + std::to_string(i);
        }
        out += ")\n{\n";
        out += "    try {\n";
        if (ex.ret_c.empty()) out += "        ";
        else                  out += "        return (long long) ";
        out += ex.name + "(";
        for (size_t i = 0; i < ex.param_c.size(); ++i) {
            if (i) out += ", ";
            out += "(" + ex.param_c[i] + ") p" + std::to_string(i);
        }
        out += ");\n";
        if (ex.ret_c.empty()) out += "        return 0;\n";
        out += "    } catch (...) {\n"
               "        std::fprintf(stderr, \"cpp2: legacy exception escaped at bridge"
               " boundary ('" + ex.name + "')\\n\");\n"
               "        std::exit(101);\n"
               "    }\n"
               "}\n";
