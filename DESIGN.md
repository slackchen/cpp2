...
4. **编译模型与速度**:模块优先。**单文件 `.cppm` 机制(类似 C#),完全不需要 `.h`**;`.h` 只在与旧 C++ 互操作时出现。
...
| P5 | **模块优先** | 没有文本包含,没有宏跨模块;编译单元 = 一个 `.cppm` 文件 |
...
| D2 | 模块机制:单文件 `.cppm`,类似 C#;`.h` 仅作互操作桥接产物 | 已确认 |
| D3 | 安全检查默认开启,提供显式退出通道 | 已确认 |
| D4 | 错误处理:错误即值 + `?` 传播 | 本稿提议 |
| D5 | 实现策略:先做转译器(.cppm → C++23)再考虑原生后端 | 本稿提议 |
...
运行:`cpp2 run hello.cppm`。没有头文件,没有 `#include`,没有前向声明。
...
> 本章是本设计的头号特性,对应目标 4:**默认模块化,单文件 `.cppm` 机制,类似 C#;完全不需要 `.h`,`.h` 只在兼容旧 C++ 时出现。**

### 3.1 单文件单元 `.cppm`

- **一个 `.cppm` 文件 = 一个模块单元**。文件顶部声明所属模块:`module app.config;`
- 允许省略模块声明,此时模块名 = 文件名(便于单文件脚本)。
- **多个文件可以声明同一个模块名**,它们是同一模块的片段(fragment),彼此完全可见、**顺序无关**——不需要 C++20 模块分区的仪式感,也不需要 `mod.rs` 式的登记。
- 目录结构只是组织手段,不隐式影响模块名。

```cpp
// app/math/vector.cppm
module app.math;
...
| 头文件 `.h`/`.hpp` | 不存在;接口由编译器从 `.cppm` 自动提取 |
| 前向声明 | 模块内顺序无关,直接使用 |
...
1. 编译器扫描 `.cppm`,提取 `export` 声明的**二进制接口**(类型 `.c2i`),缓存在 `.cpp2/ifc/<module>.c2i`。
...
```
$ cpp2 export-headers app/config.cppm -o bridge/
...
- 同一程序可自由混链 Cpp1 `.cpp` 与 C++2 `.cppm` 目标文件(同 ABI)。
...
| **桥接单元** | 含 `cxx_legacy` 块的 `.cppm`,legacy 代码的隔离防波堤 |
