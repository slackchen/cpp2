...
| `throws E` 显式类别 | ~~解析后丢弃~~ **M7b 已存档**:类别列表入函数签名(audit/BMI 可见);运行时 `err("msg", 类别码)` + match 守卫过滤已落地(见 M7b 记录) | 库定义错误 enum 与自动装箱链(v0.4) |
...
| 递归 variant(DESIGN §5.5 `list<Value>` 原例) | ~~不可直接降级~~ **M7b 定档为编译器级清晰诊断**(自引用候选点名 + 装箱指引);自动装箱经评估推迟——需转换点全链路标注,误伤面大 | 装箱语义设计单独立项 |
| 方法查找不走基类 | 字段已沿基类链;派生调用仅基类有的方法需自行转发。**虚分发已实现**(M7b):`virtual` 方法经基类智能指针多态派发(见 M7b 挂账清偿记录) | override 校验/访问控制按需评估 |
...
**M7b/M7c 完成记录(2026-08-22)**:剩余挂账清偿——
- **虚分发**:`virtual` 方法修饰符(前缀 `virtual 名字:` / 后缀 `... virtual` 双形态),发射 C++ virtual;含虚方法的类型自动合成 virtual 析构(经基指针 delete 安全);virtual 强制非 static;经基类智能指针(`unique<Animal>` 持有 Dog)箭头调用即多态派发,示例 virtual_demo 三态验证。
- **错误类别运行时**:error 增 `category`(整数码);`err("msg", 404)` 带类别构造(emit 选 err 重载);match 守卫 `err e if e.category == 404` 过滤(带守卫 err 臂允许多条,唯一性检查按守卫区分)。
- **out 出口检查(C1)**:线性近似——显式 return 与隐式末尾 return 两路均校验 out 形参已赋值;短体 + out 参数声明即报。
- **throws 类别存档(C2)**:类别列表解析入 AST 并输出至接口签名(audit/BMI 可见),不再解析后丢弃。
- **递归 variant 定档**:自引用候选编译器级清晰诊断(点名 + 装箱指引);自动装箱经评估推迟(转换点全链路标注误伤面大,单独立项)。
- **`<<`/`>>` 移位运算符**(自举暴露):词法不引入 Shl/Shr 词元(保护嵌套泛型闭合),表达式层双词元识别;优先级介于比较与算术。
- **UFCS 管道**:`v | f(a,b) ≡ f(v,a,b)`、`v | f ≡ f(v)`(纯解析层重写)。
- **lambda 显式捕获**:`[base]`/`[&x]`/`[=]`/`[&]`,与列表字面量自动消歧(']' 后跟 '(' 判定)。
- **expected 泛型转换构造**:`return "literal"` 直接受持(值语义包装)。
...
**M11 完成记录(2026-09-07)——native 后端 cxx_legacy 混动转义连接**:native 模式与 Cpp1 互操作从"模块特判"升级为通用机制;同时落地**零依赖约定**(验收标准):无 `cxx_legacy` 兼容段的程序,native 构建只需要 `cpp2.exe` 单体,不需要 C++ 编译器与 `rt/`。

- **两级方案,快路径优先**(DESIGN §9.5):① mini-C 内联不变——块内全部 `int fn(int…) { return EXPR; }` 仍直译机器码,零 C++ 工具链;② **混动卸载**——mini-C 解析不了的块,与无体声明配对的函数转译出自包含 C++ TU → `-shared` 编成 `<模块>_legacy.dll`(与 exe 同目录)→ 调用点改名 `cpp2leg_<name>` 经 PE 静态导入表直连 DLL 的 `extern "C"` 转发器。可解析/不可解析块混居时逐函数分流。
- **发射器改造**(`native/emit_win64.cpp`):`parse_legacy_block` 改返回 `bool`(结构解析 + 表达式 dry-run 复用同一 mini-C 文法,`c_expr` 增 dry 模式不落汇编),解析失败不再 unsup 而是记入失败块;`check_legacy_blocks` 按块分流——成功块照旧内联,失败块中以 `名字+'('` 归属且与无体声明配对的函数生成卸载计划(`hybrid::Plan`,`tool/src/hybrid.hpp` 共享描述);调用点单点改名(`offloaded(call_sym)` → `cpp2leg_<name>`,泛型实例名不入卸载集);`emit_native` 增计划参数,导入表归类先查计划符号→DLL 映射,再落 `dll_for` 白名单(zlib_demo 手写 asm 特判路径不变)。
- **DLL TU 生成**(`emit.cpp` `emit_legacy_dll`):prelude 与转译全局域同面(`cpp2/support.hpp` 等,legacy 原文可用同一批标准头),legacy 块原文 `#line` 映射逐字复制;每个卸载函数一个 `extern "C" __declspec(dllexport) long long cpp2leg_<name>(long long p0, …)` 转发器——签名统一 long long 槽(Win64 调用约定),体内按声明类型窄化转发、结果加宽回槽。**边界 v1 = 按值整型标量**(int/i8..u64/bool/char,`hybrid.hpp scalar_c_type` 单表);指针/引用/inout/string/throws 越界签名构建期干净拒绝并指回 `--backend=headers`。**§9.2 异常边界首步**:转发器 `try/catch(...)` → `cpp2: legacy exception escaped at bridge boundary ('<fn>')` + `exit(101)`(与契约 trap 同形),异常不穿越进 native 代码。
- **编排步**(`cmd_build.cpp`):Win64 native 分支增 DLL 编排——`emit_asm` 带出卸载计划 → needed 时惰性探测 C++ 编译器与 `rt/`(缺失则指名报错"无 cxx_legacy 兼容段的 native 构建不需要")→ 写 DLL TU → `plain_compile_command` 编 obj → `shared_link_command`(`toolchain.cpp` 新增,gcc/clang `-shared -static-libstdc++ -static-libgcc` 使 DLL 不拖编译器运行时 DLL;msvc `/LD` 文档形态)→ DLL 落 exe 同目录(PE 加载器首查路径)。`find_compiler`/`find_rt_dir` 全面惰性化:转译后端与 SysV native 分支各自探测,**纯 native Win64 路径零探测**。
